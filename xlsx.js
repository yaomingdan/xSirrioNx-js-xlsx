...
// matches <foo>...</foo> extracts content
    ...
    function parsenooplog(blob, length) {
        if (typeof console != 'undefined') console.log(blob.slice(blob.l, blob.l + length));
        blob.l += length;
    }
        ...
        if (q.HeadingPairs && q.TitlesOfParts) {
            ...
            for (var i = 0; i !== v.length; i += 2) {
                ...
                switch (v[i].v) {
                    ...
                    case "??????? ?????":
                    case "Arbeitsblatter":
                    case "Cal?sma Sayfalar?":
                    case "Feuilles de calcul":
                    case "Fogli di lavoro":
                    case "Folhas de calculo":
                    case "Planilhas":
                    case "Werkbladen":
                        p.Worksheets = len;
...
//var HIDEOBJENUM = ['SHOWALL', 'SHOWPLACEHOLDER', 'HIDEALL'];
    ...
    function parse_Window1(blob, length) {
        var xWn = blob.read_shift(2), yWn = blob.read_shift(2), dxWn = blob.read_shift(2),
    ...
    function parse_CellXF(blob, length, opts) {
        return parse_CellStyleXF(blob, length, 0, opts);
    }

    function parse_StyleXF(blob, length, opts) {
...
//	if(s + length != blob.l) throw new Error("TxO " + (s + length) + ", at " + blob.l);
                    ...
                    switch (fields[C].type) {
                        ...
                        case 'L':
                            switch (s.toUpperCase()) {
                                ...
                                case '?':
                                    out[R][C] = false;
                                    break; /* NOTE: technically unitialized */
                                default:
    ...
    var SYLK = (function () {
        ...
        function sylk_to_workbook(str, opts) {
    ...
    var PRN = (function () {
        ...
        function dsv_to_sheet_str(str, opts) {
            ...
            for (; end < str.length; ++end) switch ((cc = str.charCodeAt(end))) {
                case 0x22:
                    if (instr || (end - start == 0)) instr = !instr;
                    break;
                case sepcc:
                case 0x0a:
                case 0x0d:
        ...
        function prn_to_workbook(str, opts) {
    ...
    var parse_rs = (function parse_rs_factory() {
        var tregex = matchtag("t"), rpregex = matchtag("rPr"), rregex = /<(?:\w+:)?r>/g, rend = /<\/(?:\w+:)?r>/,
            nlregex = /\r\n/g;
    ...
    function parse_borders(t, styles, themes, opts) {
        ...
        t[0].match(tagregex).forEach(function (x) {
            ...
            switch (y[0]) {
                ...
                default:
                    if (opts && opts.WTF) throw new Error('unrecognized ' + y[0] + ' in borders');
            }
        });
    }
    ...
    function parse_fills(t, styles, themes, opts) {
        ...
        t[0].match(tagregex).forEach(function (x) {
            ...
            switch (y[0]) {
                ...
                default:
                    if (opts && opts.WTF) throw new Error('unrecognized ' + y[0] + ' in fills');
            }
        });
    }
    ...
    function parse_fonts(t, styles, themes, opts) {
        ...
        t[0].match(tagregex).forEach(function (x) {
            ...
            switch (y[0]) {
                ...
                default:
                    if (opts && opts.WTF) throw new Error('unrecognized ' + y[0] + ' in fonts');
            }
        });
    }
        ...
        t[0].match(tagregex).forEach(function (x) {
            ...
            switch (y[0]) {
                ...
                case '<ext':
                    break;
                default:
                    if (opts.WTF) throw new Error('unrecognized ' + y[0] + ' in cellXfs');
            }
        });
    }
    ...
    function write_theme(Themes, opts) {
        ...
        o[o.length] = '<a:font script="Jpan" typeface="MS P????"/>';
...
// 2.3.2
    ...
    var parse_content_xml = (function () {
        ...
        return function pcx(d, _opts) {
            ...
            while ((Rn = xlmlregex.exec(str))) switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
                ...
                case 'table':
                case '???': // 9.1.2 <table:table>
                    if (Rn[1] === '/') {
                        if (range.e.c >= range.s.c && range.e.r >= range.s.r) ws['!ref'] = encode_range(range);
                        if (merges.length) ws['!merges'] = merges;
                        sheetag.name = utf8read(sheetag['??'] || sheetag.name);
                ...
                case 'table-row':
                case '?': // 9.1.3 <table:table-row>
                    if (Rn[1] === '/') break;
                    rowtag = parsexmltag(Rn[0], false);
                    if (rowtag['??']) R = rowtag['??'] - 1; else ++R;
                    C = -1;
                    break;
    ...
    function write_zip(wb, opts) {
        ...
        // $FlowIgnore
        else wb.Props.SheetNames = wb.SheetNames.map(function (x, i) {
                return [(wb.Workbook.Sheets[i] || {}).Hidden != 2, x];
            }).filter(function (x) {
                return x[0];
            }).map(function (x) {
                return x[1];
    ...
    function readSync(data, opts) {
        ...
        switch ((n = firstbyte(d, o))[0]) {
            ...
            case 0x54:
                ...
                break;
            case 0x50:
                if (n[1] == 0x4B && n[2] < 0x20 && n[3] < 0x20) return read_zip(d, o);
                break;
            case 0xEF:
    ...
    function write_bstr_type(out, opts) {
        switch (opts.type) {
            ...
            case "binary":
                return out;
            case "file":
                return _fs.writeFileSync(opts.file, out, 'binary');
            case "buffer": {
    ...
    function writeSync(wb, opts) {
        ...
        switch (o.bookType || 'xlsb') {
            ...
            case 'txt':
                return write_bstr_type(write_txt_str(wb, o), o);
            case 'csv':
